...
describe('<TripList />', () => {
  ...
  beforeEach(async () => {
    wrapper = shallow(<TripList trip={trip} />);
  });

  it('renders trip list items', () => {
    expect(wrapper.contains(<p className="date">2019.03.21 - 2019.04.01</p>)).toBe(true);
    expect(wrapper.contains(<p className="title">제주도 먹방여행</p>)).toBe(true);
    expect(wrapper.contains(<p className="place">대한민국 제주시</p>)).toBe(true);
  });
});
